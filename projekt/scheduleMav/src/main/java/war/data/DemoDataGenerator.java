...
public class DemoDataGenerator {
	...
	public static void createAgencies() {
		...
		em.getTransaction().begin();
		Random r = new Random(0);
		for (String o : war.data.AgencyData.agencyName) {
			Agency agency = new Agency();
			agency.setAgencyName(o);
			for (String g : war.data.AgencyData.agencyName) {
				Agency a = new Agency();
				a.setAgencyName(g);
				
				Set<Agent> gAgent = new HashSet<Agent>();
				
				int amount = r.nextInt(15) + 1;
				for (int i = 0; i < amount; i++) {
					Agent newAgent = new Agent();
					newAgent.setName
					(war.data.AgencyData.manager
					[r.nextInt(war.data.AgencyData.manager.length)]);
					
					newAgent.setPhone("+358 02 555 " + r.nextInt(10) + r.nextInt(10)
							+ r.nextInt(10) + r.nextInt(10));
					newAgent.setMobile("+421 12 525 " + r.nextInt(10) + r.nextInt(10)
							+ r.nextInt(10) + r.nextInt(10));
					int n = r.nextInt(100000);
					if (n < 10000) {
						n += 10000;
					}
					newAgent.setAgency(a);
					gAgent.add(newAgent);
					em.persist(newAgent);
				}
				
				
				a.setManager(war.data.AgencyData.manager
						[r.nextInt(war.data.AgencyData.manager.length)]);
				
				
				a.setPhone("+358 02 555 " + r.nextInt(10) + r.nextInt(10)
						+ r.nextInt(10) + r.nextInt(10));
				a.setMobile("+421 12 525 " + r.nextInt(10) + r.nextInt(10)
						+ r.nextInt(10) + r.nextInt(10));
				int n = r.nextInt(100000);
				if (n < 10000) {
					n += 10000;
				...
				a.setEmail(war.data.AgencyData.mail
						[r.nextInt(war.data.AgencyData.mail.length)]);
				
				a.setEmail2(war.data.AgencyData.mail
						[r.nextInt(war.data.AgencyData.mail.length)]);
				
				a.setAddress(war.data.AgencyData.address
						[r.nextInt(war.data.AgencyData.address.length)]);
				
				a.setCity(war.data.AgencyData.city
						[r.nextInt(war.data.AgencyData.city.length)]);
				
				a.setCountry(war.data.GroupData.country
						[r.nextInt(war.data.GroupData.country.length)]);
				
				a.setZip(war.data.AgencyData.zip
						[r.nextInt(war.data.AgencyData.zip.length)]);
