...
import org.cicts.scheduler.domain.CalendarEventCIC;

import com.vaadin.ui.HorizontalSplitPanel;
import com.vaadin.ui.TabSheet;
public class SchedulerApplication extends Application {
	//public static final String PERSISTENCE_UNIT = "schedule";
  // private JPAContainer<Agency> agencies;
...
private void initLayoutContent() {
    ...
    InlineDateField dateBox = new InlineDateField();
    CalPanel3.addComponent(dateBox);
    CalPanel3.setComponentAlignment(dateBox,Alignment.MIDDLE_CENTER);
    ...
    layout.addTab(new Label("Here will be setup controls"),
	          "Setup", null );
...
private void addInitialEvents() {
    ...
    CalendarEventCIC event = getNewEvent("Whole week event", start, end);
    event.setAllDay(true);
    event.setStyleName("color4");
    event.setDescription("Description for the whole week event.");
    dataSource.addEvent(event);
    event.setPop("Scheper field");
    ...
    calendar.add(GregorianCalendar.YEAR, 2012);
    ...
    calendar.set(GregorianCalendar.MINUTE, 0);
    start = calendar.getTime();
    calendar.add(GregorianCalendar.HOUR_OF_DAY, 14);
    end = calendar.getTime();
    event = getNewEvent("MASS", start, end);
    event.setStyleName("color2");
    dataSource.addEvent(event);
